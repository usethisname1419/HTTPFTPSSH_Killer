...
def parse_arguments():
    ...
    if args.random_agent and args.service not in ['http', 'https']:
        print(f"{Fore.WHITE}[{Fore.YELLOW}ALERT{Fore.WHITE}]{Fore.RESET} Random agent module is meant for HTTP(S) attacks.")

    if args.sessions and args.service not in ['http', 'https']:
...
if __name__ == '__main__':
    try:
        ...
        if args.pause:
            verbose_print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET}{Fore.GREEN} Pause module loaded", level=2)
        ...
        for i in range(0, len(passwords), password_chunk_size):
            for idx, user in enumerate(users):
                ...
                if args.sessions:
                    ...
                    if len(users) > 1 and idx == 0:  # First user in list case
                        print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Creating a new session for the first user: {user}")
                ...
                if idx == 0:
                    if not first_iteration:
                        print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET}\n Returning to first user...\n")
                else:
