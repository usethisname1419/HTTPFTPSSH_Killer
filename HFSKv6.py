...
def parse_arguments():
    ...
    parser.add_argument('--csrf', nargs=1, help='CSRF token to include in the request')
    parser.add_argument('--cookie', nargs=1, help='Cookie value to include in the request')
...
def http_attack(ip, user, password, http_post_params, success_pattern, failure_pattern, success_content_length=None,
    ...
    try:
        ...
        if csrf_token:  # Check if csrf_token is provided
            data = data.replace('^CSRF^', csrf_token)
        else:
            data = data.replace('^CSRF^', '')  # Remove CSRF if not provided
        
        if cookie_value:  # Check if cookie_value is provided
            data = data.replace('^COOKIE^', cookie_value)
        else:
            data = data.replace('^COOKIE^', '')  # Remove Cookie if not provided
        
        verbose_print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Sending HTTP POST request to {ip} with data: {data}", level=1)
         if csrf_token:  # Check if csrf_token is provided
            verbose_print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Using cookie {cookie_value}", level=1)
        
        if cookie_value:  # Check if cookie_value is provided
            verbose_print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Updating CSRF token {csrf_token}", level=1)
        ...
        verbose_print(
            f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET}{Fore.RED} Unidentified response for [*]{Fore.YELLOW}{user}{Fore.RESET}[*]:{Fore.YELLOW}{password}", level=1)
        return None
...
def https_attack(ip, user, password, https_post_params, success_pattern, failure_pattern,
                 success_content_length=None, failure_content_length=None, proxy_ip=None, proxy_port=None):
    """ Perform an HTTPS attack by sending POST requests with given parameters."""
    ...
    try:
        ...
        # Replace placeholders in the POST data
        ...
        response = requests.post(f"https://{ip}", data=data, headers=headers, timeout=10, verify=False)
        response_length = len(response.content)

        # Print out content length and first 2000 characters of response for verbose output
        verbose_print(f"{Fore.WHITE}[{Fore.CYAN}CONTENT-LENGTH{Fore.WHITE}]{Fore.RESET}: {response_length}\n", level=1)
        verbose_print(f"{Fore.WHITE}[{Fore.CYAN}HTTPS-RESPONSE{Fore.WHITE}]{Fore.RESET}: \n{response.text[:2000]}", level=1)

        # Failure based on content length
        if failure_content_length is not None and response_length == failure_content_length:
            return False  # Indicates failure

        # Success based on content length
        if success_content_length is not None and response_length == success_content_length:
            verbose_print(
                f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Success for user {user} with password: {password}", level=1)
            return True  # Indicates success

        # Success based on pattern match in response body
        if success_pattern and success_pattern in response.text:
            verbose_print(
                f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Success for user {user} with password: {password}", level=1)
            return True  # Indicates success
        ...
        # If none of the patterns matched, return None (uncertain)
        verbose_print(
            f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET}{Fore.RED} Unidentified response for user {user} with password: {password}", level=1)
        return None
...
if __name__ == '__main__':
    try:
        ...
        if args.cookie:
            cookie_value = args.cookie  # Get the Cookie value passed through the argument
            verbose_print(f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Cookie module loaded", level=2)
        ...
        if args.service[0] == 'https':
            if not args.http_post:
                raise ValueError(
                    f"{Fore.WHITE}[{Fore.YELLOW}ERROR{Fore.WHITE}]{Fore.RESET}{Fore.RED} HTTPS POST parameters must be specified.")
            if not (args.success_pattern or args.failure_pattern):
                raise ValueError(
                    f"{Fore.WHITE}[{Fore.YELLOW}ERROR{Fore.WHITE}]{Fore.RESET}{Fore.RED} Success or failure pattern must be specified.")
