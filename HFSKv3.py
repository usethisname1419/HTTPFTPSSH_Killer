...
def https_attack(ip, user, password, https_post_params, success_pattern, failure_pattern,
    ...
    try:
        ...
        verbose_print(f"\n{Fore.WHITE}[{Fore.CYAN}HTTPS-CONTENT-LENGTH{Fore.WHITE}]{Fore.RESET}: {response_length}\n")
        ...
        if success_content_length is not None and response_length == success_content_length:
            verbose_print(
                f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET} Success for user {user} with password: {password}")
            return True  # Indicates success
        ...
        verbose_print(
            f"{Fore.WHITE}[{Fore.YELLOW}INFO{Fore.WHITE}]{Fore.RESET}{Fore.RED} Unidentified response for user {user} with password: {password}")
        return None
...
if __name__ == '__main__':
    try:
        ...
        for idx, user in enumerate(users):
            ...
            # Reset the password index for each new user
            for i in range(0, len(passwords), password_chunk_size):
                for j in range(min(iterations, len(passwords) - i)):
                    ...
                    # Save progress after each password attempt
                    save_progress(attempt_count, user, password, users[idx:], passwords[i + j + 1:])

            # Reset progress to start from the first password for the next user
            pass_idx = 0  # This line ensures that passwords start from the beginning for the next user
